...
| 2026-05-15 | rev-20260515-03 | M3 Issue 3 기준 `apps/edge-agent` smoke Dockerfile, GitHub Actions build-push workflow, OIDC ECR push role 구성 |
...
멘토링에서는 "이 서비스가 지속적인 CI/CD가 필요한 환경인지"를 먼저 설명해야 한다는 피드백이 있었다. Edge data-plane을 한 번 배포하는 것만으로는 CI/CD의 필요성이 약해 보일 수 있다.
...
> 상태: 보류
> 사유: 현재 `apps/edge-agent`는 실제 Edge data-plane 로직이 아니라 ECR/GitHub Actions 검증용 smoke image다. 이 상태에서 image tag 자동 갱신을 붙이면 아직 확정되지 않은 앱 이미지를 배포 후보로 계속 올리는 구조가 되어 운영 의미가 약하다. M4에서 `factory-a-log-adapter`, `edge-iot-publisher` 실제 로직이 확정된 뒤, 또는 M7 최종 통합 점검 전에 재개한다.
...
- M4에서 `factory-a-log-adapter`와 `edge-iot-publisher`가 smoke image가 아니라 실제 기능을 포함한다.
...
- Edge data-plane이 충분히 안정된 뒤 `build-push` 성공 후 자동 연결로 확장한다.
...
> 상태: 보류
> 사유: 현재 Issue 6 manifest tag update workflow를 Edge data-plane 실제 로직 확정 전까지 미뤘기 때문에, GitHub Actions에서 검증할 신규 배포 이벤트가 없다. 지금 만들면 배포 검증이라기보다 현재 상태 조회 workflow가 되므로 Issue 6 재개 이후 진행한다.
...
- Issue 6이 재개되어 GitOps values tag update가 자동 또는 수동 승인형 workflow로 동작한다.
- 검증 대상 배포 이벤트가 명확하다.
  - 예: `envs/factory-a/values.yaml` image tag 변경 commit
  - 또는 ArgoCD 수동 Sync 완료 이벤트
- GitHub Actions에서 ArgoCD/K3s 상태를 조회할 인증 경계가 정해져 있다.
  - ArgoCD API/CLI token
  - 또는 factory-a kubeconfig secret
  - 또는 Hub 내부 검증 playbook을 호출하는 별도 경로
...
> 상태: 보류
> 사유: 이 이슈는 `빌드 -> ECR 푸시 -> manifest 갱신 -> ArgoCD Sync -> Spoke 롤아웃` 전체 흐름 검증이므로 Issue 6 없이는 완료할 수 없다. Edge data-plane 로직과 manifest 갱신 workflow가 확정된 뒤 재개한다.
...
- Issue 6 manifest tag update workflow가 재개되어 GitOps repo에 image tag 변경 commit이 생성된다.
- Issue 7 배포 검증 workflow 또는 동등한 검증 스크립트가 준비되어 있다.
- `factory-a`에 올릴 Edge data-plane image가 smoke image가 아니라 실제 기능 검증 대상이다.

### M3 현재 종료 기준

2026-05-15 기준 M3는 다음 범위를 완료한 상태로 닫고, 남은 자동 배포 연계는 Edge data-plane 로직 확정 이후 재개한다.

완료:

- GitOps repo 구조와 factory values 기준
- ECR repository와 image tag 전략
- GitHub Actions OIDC 기반 ECR build/push
- ArgoCD ApplicationSet으로 factory-a 제어
- factory-a manual sync/rollback 정책과 bad image rollback 검증

보류:

- Issue 6 manifest tag update workflow
- Issue 7 deploy verification workflow
- Issue 8 push-to-rollout end-to-end 자동 검증

다음 마일스톤은 M4 데이터 플레인이다. `factory-a-log-adapter`와 `edge-iot-publisher`가 실제로 어떤 데이터를 만들고 IoT/S3/Lambda/DynamoDB가 어떤 계약으로 연결될지 확정한 뒤 M3 Issue 6~8을 재개한다.
