...
2026-06-01 배포 이후 `GRAPH#5M`은 기존 전체 node 평균 필드와 함께 `infra.nodes[]`에 node별 5분 집계를 저장한다.
...
sk BETWEEN GRAPH#5M#{from_bucket} AND GRAPH#5M#{to_bucket}
...
- `factory-a`는 node 3개, `factory-b/c`는 node 2개다. Backend는 `infra.nodes[]` 길이를 고정값으로 가정하지 않는다.
