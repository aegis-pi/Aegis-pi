...
> **마일스톤 목표**: `factory-a` Spoke의 센서 데이터가 Edge Agent → IoT Core → S3까지 실제로 흐르는 것을 검증한다.  
...
- [ ] source_type별 payload 구조 확정 및 샘플 작성
  ...
  - `system_status`: `node_status`, `edge_agent_status`, `input_module_status`, `camera_status`, `mic_status`
...
- [ ] 수집 주기 설정 (주기값은 `docs/ops/test_checklist.md` 기반 테스트 후 확정)
...
- 2개 이상의 `source_type` 경로에 파일이 분리 적재됨
...
- [ ] source_type별 경로 분리 적재 확인 (`sensor`, `system_status`)
- [ ] 정규화 서비스 처리 확인
- [ ] `pipeline_status` 집계 동작 확인
- [ ] 데이터 지연/누락 발생 시 `pipeline_status` 이상 판정 확인
- [ ] 검증 결과를 데이터 플레인 관련 문서와 `docs/ops/test_checklist.md`에 반영

### 🔍 Acceptance Criteria

- S3에서 `factory-a` 데이터 주기적 적재 확인 (최소 10분 이상 연속)
- `source_type` 2개 이상 경로에 데이터 분리 적재 확인
