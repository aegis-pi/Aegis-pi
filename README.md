...
| 현재 완료 범위 | M0 `factory-a`, M1 Issue 0~8 완료. IoT Rule -> S3 raw 적재, IRSA S3/AMP 권한, Hub Prometheus Agent -> AMP remote_write, 내부 Grafana -> AMP query 검증 완료 |
| 현재 AWS 상태 | 2026-05-06 `build-hub` 완료. Hub EKS/VPC/NAT/EIP, ArgoCD, Prometheus Agent, 내부 Grafana, foundation S3/AMP/IoT Rule, `factory-a` IoT/K3s Secret active 확인. 과거 KMS keys는 `PendingDeletion` |
| 다음 작업 | M1 Issue 9 AWS Load Balancer Controller 준비, 이후 Issue 10 ArgoCD/Grafana HTTPS Admin Ingress 구성 |
| 비용 기준 | 현재 Hub active 고정 비용 `~$0.32/hour` 기준. 상세 기준은 `docs/ops/15_aws_cost_baseline.md` |
...
M1 Issue 0~8에서는 AWS MFA/Terraform 접근, Hub EKS/VPC 기준선, Hub namespace 기준선, Hub ArgoCD bootstrap, foundation S3 data bucket `aegis-bucket-data`, AMP Workspace `AEGIS-AMP-hub`, `factory-a` IoT Thing/certificate/policy/K3s Secret, IoT Rule -> S3 `raw/` prefix 적재, `risk/risk-normalizer` IRSA S3 권한, `observability/prometheus-agent` 설치 및 AMP remote_write 수신, 내부 Grafana AMP datasource query를 검증했다.

2026-05-06 기준 `scripts/build/build-all.sh` 실행으로 Hub와 foundation 리소스는 active 상태다. 전체 재생성은 `scripts/build/build-all.sh`, 전체 삭제는 `scripts/destroy/destroy-all.sh`를 사용한다. 비용 기준은 `docs/ops/15_aws_cost_baseline.md`를 따른다.
...
| 경로 | 역할 | 현재 상태 |
| --- | --- | --- |
| `infra/hub` | VPC, subnet, NAT Gateway, EKS cluster, node group, EKS OIDC 기반 IRSA | active, Terraform state present |
| `scripts/ansible/playbooks` | kubeconfig 갱신, namespace, LimitRange, ArgoCD Helm install, Prometheus Agent remote_write, Grafana AMP datasource bootstrap/검증 | active, rebuild 시 재실행 |
...
| Phase 2 (M1) | AWS EKS Hub 기준선 구성 | 진행 중, Issue 0~8 완료, Issue 9 대기 |
