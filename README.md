...
| 현재 완료 범위 | M0 `factory-a`, M1 Issue 0~2 |
| 현재 AWS 상태 | Hub EKS/VPC/namespace 기준선 검증 후 비용 절감을 위해 destroy 완료 |
| 후속 범위 | Hub ArgoCD, Edge Agent, Dashboard VPC, `factory-b`, `factory-c`, IoT Core, S3, Risk Twin |
...
M1 Issue 0~2에서는 AWS MFA/Terraform 접근, Hub EKS/VPC 기준선, Hub namespace 기준선을 검증했다. 테스트로 생성했던 AWS 리소스는 현재 `terraform destroy`로 제거되어 비용이 발생하지 않는 상태다.

Terraform root는 최소 분리 기준으로 나뉜다.

| 경로 | 역할 | 현재 상태 |
| --- | --- | --- |
| `infra/hub` | VPC, subnet, NAT Gateway, EKS cluster, node group | validate 완료, state empty |
| `infra/platform` | Kubernetes namespace, LimitRange, 이후 ArgoCD/관측 컴포넌트 | validate 완료, EKS 생성 후 apply |
| `infra/foundation` | S3, ECR, AMP, IoT Core처럼 EKS destroy와 분리할 영속 리소스 | 자리만 생성 |
...
| Phase 2 (M1) | AWS EKS Hub 기준선 구성 | 진행 중, Issue 0~2 완료 |
...
14. `docs/issues/M1_hub-cloud.md`
15. `docs/ops/13_hub_namespace_baseline.md`
