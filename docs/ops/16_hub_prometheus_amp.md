...
- 데이터 수집/처리의 source of truth는 AMP가 아니라 IoT Core -> Lambda data processor -> DynamoDB LATEST/HISTORY#STATE + S3 processed, DataProcessorRefresh1m -> stale `pipeline_status`/`risk` refresh, GraphAggregator5m -> DynamoDB GRAPH#5M + S3 processed_agg이다.
