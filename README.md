...
| 후속 범위 | Edge Agent, AWS Hub, `factory-b`, `factory-c`, IoT Core, S3, Risk Twin |
...
후속 확장에서는 `edge-agent`를 추가해 `factory-a` 로컬 데이터를 AWS IoT Core로 송신하고, IoT Core, S3, AMP/Grafana, Risk Score Engine, ApplicationSet 기반 배포를 추가한다.
...
10. `docs/issues/M0_factory-a_safe-edge-baseline.md`
