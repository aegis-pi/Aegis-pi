...
| 현재 단계 | M6 Risk 데이터 계약 고도화 및 MVP Daily Factory Report 검증 완료 상태 |
...
| 현재 AWS 상태 | 2026-05-28 기준 Foundation/IoT/data-pipeline 리소스는 보존/검증 상태다. S3 `aegis-bucket-data`는 versioning/SSE-S3/public access block/lifecycle 적용. IoT Rule 3개는 Lambda + S3 raw action 연결. Lambda `AEGIS-Lambda-DataProcessor` Active/Successful. DynamoDB `AEGIS-DynamoDB-FactoryStatus`는 `factory-a/b/c` LATEST 갱신 및 TTL 활성. Reporting stack은 수동 검증 후 `destroy-reporting.sh`로 삭제했으며 S3 `processed/` 입력과 `reports/daily/` 산출물은 보존된다 |
| 다음 작업 | Dashboard page 및 Dashboard VPC는 별도 담당 범위로 분리한다. 현재 repo 쪽 우선순위는 Risk output 계약 고정, `runtime-config.yaml`을 Lambda Risk 계산에 연결, Risk Twin 출력 구조를 DynamoDB/S3 processed에 명확히 반영, Daily Report S3 read 성능/비용 개선, Bedrock token usage/object count 관측값 저장이다 |
