...
| 현재 완료 범위 | M0 `factory-a`, M1 Issue 0~3, M1 Issue 4 S3 bucket 생성 |
| 현재 AWS 상태 | Hub EKS/VPC/namespace/ArgoCD는 destroy 완료, foundation S3 bucket은 active |
| 후속 범위 | IoT Core, Edge Agent, Dashboard VPC, `factory-b`, `factory-c`, Risk Twin |
...
M1 Issue 0~3에서는 AWS MFA/Terraform 접근, Hub EKS/VPC 기준선, Hub namespace 기준선, Hub ArgoCD bootstrap을 검증했다. 테스트로 생성했던 Hub EKS 리소스는 현재 `terraform destroy`로 제거되어 비용이 발생하지 않는 상태다. M1 Issue 4에서는 `infra/foundation`의 S3 data bucket `aegis-bucket-data`를 생성했다.
...
| 경로 | 역할 | 현재 상태 |
| --- | --- | --- |
| `infra/hub` | VPC, subnet, NAT Gateway, EKS cluster, node group | validate 완료, state empty |
| `scripts/ansible/playbooks/hub_argocd_bootstrap.yml` | kubeconfig 갱신, namespace, LimitRange, ArgoCD Helm install, 검증 | syntax 기준 추가 |
| `infra/foundation` | S3, ECR, AMP, IoT Core처럼 EKS destroy와 분리할 영속 리소스 | S3 data bucket apply 완료 |
...
| Phase 2 (M1) | AWS EKS Hub 기준선 구성 | 진행 중, Issue 0~3 완료 |
