...
| Terraform root split | `infra/hub`, `infra/platform`, `infra/foundation` | EKS 자체와 클러스터 내부 리소스, 영속 데이터 리소스의 생명주기를 분리한다. |
...
최소 분리 기준의 Terraform root는 다음 구조를 따른다.
...
2026-04-30에 기존 인프라를 제거한 뒤 `infra/hub` Terraform root에서 `terraform apply`를 다시 실행했다. 이후 `infra/platform`에서 namespace와 LimitRange를 적용했다.

결과:

```text
infra/hub + infra/platform apply: 64 added, 0 changed, 0 destroyed.
final destroy: 64 destroyed.
```
