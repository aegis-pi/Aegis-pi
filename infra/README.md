...
| `data-pipeline/` | IoT Rule × 3 (factory-a/b/c), Lambda DataProcessor, DataProcessorRefresh1m, GraphAggregator5m, EventBridge Scheduler, IAM. on-demand 레이어. `build-data-pipe.sh` / `destroy-data-pipe.sh`로 개별 관리 |
...
- Hub 재생성은 `infra/hub` Terraform apply 후 `scripts/ansible/playbooks/hub_argocd_bootstrap.yml`, `hub_prometheus_agent_cleanup.yml`, `hub_grafana_bootstrap.yml`, `hub_aws_load_balancer_controller_bootstrap.yml` 순서로 진행한다. Admin UI Ingress와 Spoke 등록은 별도 후속 스크립트로 실행한다.
