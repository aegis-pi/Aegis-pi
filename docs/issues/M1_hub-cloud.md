...
> **마일스톤 목표**: EKS 클러스터와 AWS 관리형 서비스(IoT Core, S3, AMP)를 구성한다.  
...
이 단계가 완료되어야 Grafana Hub에서 AMP를 실제 데이터 소스로 사용할 수 있다.
...
본사 관제 담당자가 사용할 중앙 관제 UI를 EKS에 구성한다.  
이 단계에서는 AMP 데이터 소스를 연결하는 것까지 완료하고,  
실제 대시보드 구성은 M6(Risk Twin + 관제 화면)에서 진행한다.

### ✅ 완료 조건 (Definition of Done)

- [ ] `observability` 네임스페이스에 Grafana 설치
- [ ] Grafana 초기 admin 비밀번호 설정
- [ ] Grafana UI 접근 가능 확인
- [ ] 데이터 소스 연결
  - AMP (Prometheus 호환)
- [ ] Grafana 버전 및 설치 방식 기록
- [ ] Timestream 연동은 Risk Twin 저장소 결정 이후 후속 단계(M6)에서 진행한다고 문서에 명시

### 🔍 Acceptance Criteria

- Grafana UI 브라우저 접근 가능
