...
본사 관리자용 최종 화면은 Dashboard VPC의 Web/API로 분리한다. 다만 MVP 단계에서는 관리자들이 실제 HTTPS 경로로 Grafana UI에 접근할 수 있는지 확인하기 위해 M1 Issue 10에서 별도 Admin Ingress를 구성한다.
...
- 현재 구현은 외부 LoadBalancer, Ingress, ALB, public DNS를 만들지 않는다.
- 현재 로컬 접근은 `kubectl port-forward`를 사용한다.
- M1 Issue 10에서는 Service를 `ClusterIP`로 유지한 채 Public ALB 1개와 HTTPS host 기반 Ingress로 관리자 접근을 검증한다.
...
| 화면 | 대상 사용자 | 접근 방식 | 데이터 |
| --- | --- | --- | --- |
| 내부 Grafana | 운영자/개발자 | EKS kubeconfig + port-forward, 후속 HTTPS Admin Ingress | AMP 메트릭 |
...
Grafana Pod 자체는 기존 EKS worker node 위에서 실행되므로 별도 AWS 고정 시간 비용을 추가하지 않는다. 현재 구현은 외부 Load Balancer를 만들지 않기 때문에 ALB/NLB 비용도 없다. M1 Issue 10에서 HTTPS Admin Ingress를 적용하면 공유 Public ALB, public IPv4, Route53 쿼리/hosted zone 비용 기준을 `docs/ops/15_aws_cost_baseline.md`에 추가한다.
