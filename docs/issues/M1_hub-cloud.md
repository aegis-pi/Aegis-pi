...
각 Spoke의 `edge-iot-publisher`가 데이터를 전송할 IoT Core 진입점을 구성한다.
...
- 인증서 파일이 안전하게 보관됨 (`edge-iot-publisher` 배포 시 사용)
...
- 상태: 완료
- 진행 요약: `factory-a` `edge-iot-publisher`가 사용할 IoT Core Thing, 인증서, 정책, K3s Secret, IoT Rule을 구성해 Edge -> IoT Core -> S3 raw 적재 앞단을 완성했다.
...
- 후속: 실제 Edge data-plane 송신 구현과 end-to-end 데이터 플레인 검증은 M4에서 진행한다.
