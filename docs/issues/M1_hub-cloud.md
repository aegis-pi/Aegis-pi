...
- 다음 작업: Issue 6의 AMP Workspace 생성 및 Prometheus/Agent remote_write 권한 준비. Issue 7에서 Prometheus Agent 실제 전송까지 완료.
...
- 다음 작업: Issue 7 Prometheus Agent 실제 전송 검증과 Issue 8 내부 Grafana/AMP 기준 결정은 완료. 이후 Issue 9에서 AWS Load Balancer Controller를 준비하고, Issue 10에서 ArgoCD/Grafana HTTPS Admin Ingress를 구성한다.
...
- 후속: Prometheus Agent 설치와 실제 remote_write 메트릭 수신 검증은 Issue 7에서 완료했고, 내부 Grafana/AMP 기준은 Issue 8에서 완료했다. 다음은 Issue 9에서 AWS Load Balancer Controller를 준비한다.
...
- 후속: 내부 관측용 Grafana/AMP 데이터 소스 기준은 Issue 8에서 결정했고, 관리자 HTTPS 접근 경로는 Issue 9~10에서 별도 구성한다.
...
본사 관리자용 최종 대시보드는 Dashboard VPC의 Web/API로 확장한다. 다만 MVP 단계에서는 관리자들이 ArgoCD와 Grafana 웹 UI에 실제 HTTPS 경로로 접근할 수 있는지 검증하기 위해 Issue 9~10에서 별도 Admin Ingress를 구성한다.
...
- 후속 접근 계획: Issue 10에서 `grafana` Service는 `ClusterIP`로 유지하되, Public ALB + HTTPS Ingress host rule로 관리자 외부 접근을 검증한다.
- Verification:
...
- 후속: M1 Issue 9에서 AWS Load Balancer Controller를 준비하고, Issue 10에서 ArgoCD/Grafana HTTPS Admin Ingress를 구성한다.
...
- [ ] AWS Load Balancer Controller용 IAM policy와 IRSA role 구성
- [ ] `kube-system` 또는 지정 namespace에 AWS Load Balancer Controller Helm release 설치
- [ ] Hub public subnet에 ALB discovery tag 확인 또는 보강
  - `kubernetes.io/role/elb = 1`
  - `kubernetes.io/cluster/AEGIS-EKS = shared` 또는 controller가 요구하는 동등 기준
- [ ] EKS OIDC provider와 controller ServiceAccount annotation 연결 확인
- [ ] `scripts/build/build-hub.sh`에서 재생성 시 controller가 자동 설치/검증되도록 연결
- [ ] `scripts/destroy/destroy-hub.sh` 또는 관련 destroy 흐름에서 controller/Ingress 생성 ALB가 삭제되는 순서 확인
- [ ] 운영 문서에 controller 책임 경계와 비용 영향 기록
...
- 상태: 대기
- 진행 요약: ArgoCD/Grafana HTTPS Admin Ingress를 만들기 위한 AWS Load Balancer Controller 선행 작업으로 분리했다.
- 변경/확인: 예정 파일은 `infra/hub`, `scripts/ansible`, `scripts/build`, `scripts/destroy`, `docs/ops` 기준으로 정리한다.
- 검증: 미실행
- 후속: Issue 10에서 실제 ArgoCD/Grafana HTTPS host rule을 구성한다.
...
- [ ] Admin UI용 도메인과 ACM public certificate 기준 결정
  - 예: `argocd.<domain>`, `grafana.<domain>`
  - ACM 인증서는 ALB/Route53에 쓰는 non-exportable public certificate 기준
- [ ] Public ALB 1개를 공유하는 Ingress group 기준 정의
- [ ] ArgoCD Ingress host rule 작성
- [ ] Grafana Ingress host rule 작성
- [ ] HTTPS listener `443`만 외부 진입점으로 사용
- [ ] HTTP `80`은 비활성화하거나 HTTPS redirect 용도로만 사용
- [ ] 관리자 IP allowlist를 MVP 최소 보호선으로 적용
- [ ] ArgoCD/Grafana 자체 로그인은 유지
- [ ] Route53 record 생성 또는 수동 DNS 연결 절차 문서화
- [ ] `build-all`/`build-hub` 재실행 시 Admin Ingress가 재현되는지 확인
- [ ] `destroy-all`/`destroy-hub` 실행 시 Ingress가 만든 ALB/TargetGroup/SecurityGroup이 삭제되는지 확인
- [ ] 비용 문서에 ALB, public IPv4, Route53, ACM 비용 기준 반영
...
- 상태: 대기
- 진행 요약: MVP에서는 WAF/Cognito/OIDC 없이 ArgoCD/Grafana의 HTTPS 외부 접근 가능 여부를 검증하는 범위로 정의했다.
- 변경/확인: 예정 파일은 Admin Ingress manifest/template, Ansible bootstrap/verify, build/destroy 흐름, 비용 문서 기준으로 정리한다.
- 검증: 미실행
- 후속: 운영 보안 강화가 필요해지면 Issue 11에서 WAF/Cognito/OIDC를 붙인다.
...
- 검증: 미실행
- 후속: Issue 10의 HTTPS Admin Ingress 검증 후 운영 전환 필요성이 생기면 진행한다.
