...
상태: historical source of truth for M1
기준일: 2026-04-30

최신 기준: 2026-05-09 기준 확정 클라우드 아키텍처와 VPC 명명은 `docs/planning/15_cloud_architecture_final.md`를 따른다. 이 문서의 `Hub Processing VPC` 표현은 현재 2번 `Control / Management VPC`의 초기 M1 구현 기준으로 해석한다.
...
이 결정은 Hub VPC와 EKS 클러스터의 MVP 기준, 2026-04-30 적용/검증 결과, 그리고 최소 Terraform root 분리 기준을 다룬다. 최신 명칭으로는 2번 Control / Management VPC의 초기 구현 기록이다.
...
| VPC CIDR | `10.0.0.0/16` | Hub VPC 기본 대역으로 고정한다. 최신 명칭으로는 2번 Control / Management VPC 대역이며, 1번 Data / Dashboard VPC나 Spoke CIDR과는 별도 대역으로 관리한다. |
...
| Instance type | `t3.medium` 기본, 필요 시 `t3.large` | MVP 제어/관측 컴포넌트 실행은 `t3.medium`으로 시작한다. 최신 기준에서는 ArgoCD/Grafana는 2번 Control / Management VPC, Risk 계산은 Lambda data processor와 DynamoDB/S3 processed로 분리한다. |
