...
| Instance type | `t3.medium` 기본, 필요 시 `t3.large` | MVP 제어/관측 컴포넌트 실행은 `t3.medium`으로 시작한다. 최신 기준에서는 ArgoCD/Grafana는 2번 Control / Management VPC, Risk Engine은 1번 Data / Dashboard VPC 처리 영역으로 분리한다. |
