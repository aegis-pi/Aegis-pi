...
> **마일스톤 목표**: 수집된 데이터를 기반으로 Risk Score를 계산하고 본사 관제 담당자가 사용할 Grafana 대시보드를 완성한다.  
...
- [ ] 기준값을 `docs/ops/test_checklist.md`에 보정 대상으로 기록
...
MVP 단계에서는 별도 저장소를 추가하지 않고, Risk Twin 결과를 Prometheus 호환 메트릭으로 노출하여 AMP/Grafana 축에 바로 연결한다.
...
- [ ] 출력 결과를 Prometheus 호환 메트릭으로 노출
  - 예: `risk_score`, `risk_status`, `risk_cause_weight`, `risk_cause_rank`
- [ ] AMP/Grafana에서 조회 가능한 메트릭 구조로 정리
...
- Risk Twin 결과가 Prometheus 메트릭으로 노출됨 확인
- 3개 공장 각각 독립 출력 확인
...
관제 담당자가 가장 먼저 보는 상단 위험도 카드를 Grafana에서 구현한다.  
각 공장의 현재 상태, 변화 방향, 이상 시스템 개수를 한눈에 파악할 수 있어야 한다.  
이 패널은 Risk Twin에서 노출한 메트릭을 기준으로 구성한다.

### ✅ 완료 조건 (Definition of Done)

- [ ] Grafana 대시보드 생성 (본사 관제 메인)
...
- Grafana 대시보드에서 3개 공장 위험도 카드 확인
...
이상 시스템 목록은 Risk Twin 메트릭 기반으로 구성한다.
...
- [ ] 이상 시스템 목록 패널 구현 (중단 오른쪽)
  ...
  - 데이터 소스: Risk Twin 메트릭 (`top_causes`, 상태 메트릭)

### 🔍 Acceptance Criteria

- Grafana에서 3개 공장 온도/습도 추세선 확인
...
- [ ] 검증 결과를 Risk 관련 문서와 `docs/ops/test_checklist.md`에 반영

### 🔍 Acceptance Criteria

- 5개 시나리오 전부 예상 Risk Score 범위 내 결과 확인
- 기준값 초과/복구에 따라 위험도 카드 상태가 기대대로 변경됨 확인
- Grafana 관제 화면이 Score 변화에 따라 실시간 갱신
