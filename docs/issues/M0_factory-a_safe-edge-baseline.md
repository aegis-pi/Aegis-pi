...
> 현재 M0의 핵심 기준선은 구축 및 장애 테스트까지 완료됐다. AWS Hub EKS/VPC/namespace/ArgoCD bootstrap, AWS Load Balancer Controller, Admin UI HTTPS Ingress, foundation S3/AMP/IoT Rule, `factory-a` IoT/K3s Secret은 build 스크립트로 재생성/검증 가능하며, `factory-b`, `factory-c` 확장은 후속 단계다.
