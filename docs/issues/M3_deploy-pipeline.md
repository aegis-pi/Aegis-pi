...
> 상태: 보류
> 사유: 현재 Issue 6 manifest tag update workflow를 Edge Agent 실제 로직 확정 전까지 미뤘기 때문에, GitHub Actions에서 검증할 신규 배포 이벤트가 없다. 지금 만들면 배포 검증이라기보다 현재 상태 조회 workflow가 되므로 Issue 6 재개 이후 진행한다.
...
> 상태: 보류
> 사유: 이 이슈는 `빌드 -> ECR 푸시 -> manifest 갱신 -> ArgoCD Sync -> Spoke 롤아웃` 전체 흐름 검증이므로 Issue 6 없이는 완료할 수 없다. Edge Agent 로직과 manifest 갱신 workflow가 확정된 뒤 재개한다.
...
- `factory-a`에 올릴 Edge Agent image가 smoke image가 아니라 실제 기능 검증 대상이다.

### M3 현재 종료 기준

2026-05-15 기준 M3는 다음 범위를 완료한 상태로 닫고, 남은 자동 배포 연계는 Edge Agent 로직 확정 이후 재개한다.
...
다음 마일스톤은 M4 데이터 플레인이다. Edge Agent가 실제로 어떤 데이터를 만들고 IoT/S3/Lambda/DynamoDB가 어떤 계약으로 연결될지 확정한 뒤 M3 Issue 6~8을 재개한다.
