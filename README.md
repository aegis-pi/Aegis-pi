...
| 현재 완료 범위 | M0 `factory-a`, M1 Issue 0~3 |
| 현재 AWS 상태 | Hub EKS/VPC/namespace/ArgoCD bootstrap 기준선 검증 후 비용 절감을 위해 destroy 완료 |
| 후속 범위 | S3, IoT Core, Edge Agent, Dashboard VPC, `factory-b`, `factory-c`, Risk Twin |
...
M1 Issue 0~3에서는 AWS MFA/Terraform 접근, Hub EKS/VPC 기준선, Hub namespace 기준선, Hub ArgoCD bootstrap을 검증했다. 테스트로 생성했던 AWS 리소스는 현재 `terraform destroy`로 제거되어 비용이 발생하지 않는 상태다.
