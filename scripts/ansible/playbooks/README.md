...
| `hub_tailscale_bootstrap.yml` | Tailscale Operator, Spoke egress Service, ArgoCD/Grafana Tailscale UI Service, ArgoCD Spoke cluster Secret 적용 |
| `hub_tailscale_verify.yml` | Tailscale Operator/proxy readiness, Spoke K3s API TCP, Tailscale UI HTTP, ArgoCD cluster Secret 설정 확인 |
| `hub_aegis_spoke_applicationset_bootstrap.yml` | GitOps repo URL 변수 기준 AEGIS Spoke ApplicationSet 적용 및 enabled Spoke Application 생성 확인 |
| `hub_aegis_spoke_applicationset_verify.yml` | ApplicationSet repo URL과 enabled Spoke Application destination 확인 |
...
- Hub Tailscale bootstrap은 `aegis_spokes`에서 `enabled: true`인 Spoke만 대상으로 실행한다. `factory-a`가 offline이면 임시로 `enabled: false`로 제외하고 `factory-b/c`만 등록/검증할 수 있다. `~/Aegis/.aegis/secrets/tailscale/operator.env`가 없으면 해당 단계가 실패한다.
...
- 현재 ApplicationSet 기본 scope는 `envs/factory-*/values.yaml`이고, destination namespace는 `ai-apps`다. 특정 factory만 관리하려면 `AEGIS_GITOPS_APPSET_VALUES_GLOB='envs/factory-b/values.yaml'`처럼 override한다.
- `BUILD_TAILSCALE=true` 기준 `build-iot-factory-a.sh`는 Tailscale bootstrap/verify 이후 `hub_aegis_spoke_applicationset_bootstrap.yml`과 `hub_aegis_spoke_applicationset_verify.yml`를 이어서 실행한다.
