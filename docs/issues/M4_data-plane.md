...
S3 raw 적재 흐름은 유지하되, Dashboard가 조회할 최신 상태는 별도 latest status 경로로 반영하는 방향을 추가 검토한다.
...
- factory별 전송 주기
- 초당 메시지 수
- IoT Core 수신 후 latest status 반영 지연
- 10분 이상 연속 송신 기준 수신 성공률/실패율
- S3 raw와 latest status store 양쪽 경로 검증
...
- [ ] 경로 파티셔닝 규칙 적용 확인
  - `s3://bucket/raw/factory-a/sensor/yyyy=2026/mm=04/dd=24/<message_id>.json`
  - `s3://bucket/raw/factory-a/system_status/yyyy=2026/mm=04/dd=24/<message_id>.json`
  - `s3://bucket/raw/factory-a/device_status/yyyy=2026/mm=04/dd=24/<message_id>.json`
  - `s3://bucket/raw/factory-a/workload_status/yyyy=2026/mm=04/dd=24/<message_id>.json`
  - `s3://bucket/raw/factory-a/pipeline_heartbeat/yyyy=2026/mm=04/dd=24/<message_id>.json`
...
- S3 콘솔에서 `raw/factory-a/sensor/` 경로에 파일 적재 확인
- 적재된 파일 내용이 표준 스키마와 일치
- 4개 이상의 `source_type` 경로에 파일이 분리 적재됨

---

## Issue 6 - [데이터/정규화] 정규화/판단 서비스 구현

### 🎯 목표 (What & Why)

S3에 적재된 원본 데이터를 읽어 정규화하고 Risk Score Engine에 전달하는 서비스를 구현한다.  
최신 아키텍처 기준의 운영 배치는 1번 Data / Dashboard VPC 처리 영역이며, 기존 Hub `risk` 네임스페이스는 M1 검증용 또는 임시 workload 영역으로만 본다.
