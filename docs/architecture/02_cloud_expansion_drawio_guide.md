...
```text
                    ...
                    AWS Cloud / EKS Hub
        +------------------------------------------------+
        | ArgoCD | Grafana | Risk Services | Ops Support |
        | IoT Core | S3 | ECR | AMP                  |
        +------------------------------------------------+
             ^              ^              ^
             |              |              |
...
| Risk Score Engine | AMP/Grafana | Risk Twin 메트릭 노출 및 조회 |
| pipeline-status-aggregator | IoT Core / S3 | 수신/적재 상태 확인 |
| Grafana Hub | AMP / Risk metrics | 중앙 관제 조회 |

### draw.io 권장 형태

- AWS Cloud는 가장 큰 박스로 둔다.
- EKS Hub는 AWS Cloud 안의 큰 박스로 둔다.
- IoT Core, S3, ECR, AMP는 EKS 바깥의 AWS managed service로 둔다.
- ArgoCD, Grafana, Risk Normalizer, Risk Score Engine, pipeline-status-aggregator는 EKS 안에 둔다.
...
| Risk Score Engine | AMP | Prometheus metrics |
| Grafana Hub | AMP | query |
...
- [ ] AWS Cloud / EKS Hub / 3개 Spoke / GitHub 경계가 보인다.
- [ ] Hub가 중앙 배포, 중앙 수집, 중앙 관제를 담당하는 것이 보인다.
...
- [ ] Grafana가 Risk Twin 결과를 조회하는 구조가 보인다.
...
draw.io에서는 한 파일 안에 탭 4개를 두는 방식을 권장한다.
