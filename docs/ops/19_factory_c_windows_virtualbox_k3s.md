...
- `infra_state.payload`: `heartbeat`, `cluster`, `nodes` (**길이 2**: master + worker), `workloads`, `devices`
...
# server 설치 + Tailscale IP 를 TLS SAN 에 포함, flannel 인터페이스 및 node-ip를 host-only 어댑터(enp0s8, 192.168.56.10)로 지정하여 NAT IP 충돌 방지
curl -sfL https://get.k3s.io | \
  INSTALL_K3S_EXEC="server --node-name factory-c-master --tls-san ${TS_IP_MASTER} --node-ip 192.168.56.10 --flannel-iface enp0s8" \
  sh -
...
curl -sfL https://get.k3s.io | \
  ...
  K3S_TOKEN="${K3S_TOKEN}" \
  INSTALL_K3S_EXEC="agent --node-name factory-c-worker --node-ip 192.168.56.20 --flannel-iface enp0s8" \
  sh -
...
### 11-1. K3s 버전을 환경 파일에 박아 두기

worker VM 에는 kubectl 이 기본으로 없다 (k3s agent install 은 server 처럼 kubectl 심볼릭 링크를 만들지 않음). 매번 cluster 에 질의하지 않도록 K3s 버전을 install 시점에 한 번만 채집해 파일로 저장한다.

```bash
# worker VM
sudo mkdir -p /etc/aegis /var/lib/aegis
K3S_VER="$(/usr/local/bin/k3s --version | awk '/k3s version/ {print $3}')"
echo "${K3S_VER}"   # 예: v1.34.6+k3s1

sudo tee /etc/aegis/dummy-publisher.env >/dev/null <<EOF
K3S_VERSION=${K3S_VER}
EOF

sudo chmod 600 /etc/aegis/dummy-publisher.env
```

> K3s 업그레이드 후에는 이 파일을 수동 갱신한다.

### 11-2. 더미 publisher 스크립트 작성

`/usr/local/bin/aegis-dummy-publisher.sh` 는 `noisy-vm` 프로파일대로 값을 흩어 두 토픽으로 publish 하는 무한 루프다. `infra_state.payload.nodes` 배열에 master / worker 두 노드를 함께 보낸다.
...
publish_factory_state() {
  ...
  jq -n \
    ...
    '{
      ...
      payload: {
        aggregation_window_seconds: 3,
        sensor: { sample_count: 1, temperature_celsius_avg: $t, humidity_percent_avg: $h, pressure_hpa_avg: $p },
        ai_result: { sample_count: 1, fire_score: $fire, fall_score: $fall, bend_score: $bend, abnormal_sound: $sound }
      }
...
publish_infra_state() {
  local ts seq
  ts="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  seq="$(cat "${SEQ_FILE}")"; seq=$((seq + 1)); echo "${seq}" > "${SEQ_FILE}"

  # master / worker 노드 메트릭 (synthetic — testbed 용)
  local s_cpu s_mem s_disk a_cpu a_mem a_disk
  s_cpu="$(rand_jitter 8.0 3.0)";  s_mem="$(rand_jitter 30.0 5.0)";  s_disk="$(rand_jitter 22.0 2.0)"
  a_cpu="$(rand_jitter 12.0 4.0)"; a_mem="$(rand_jitter 38.0 6.0)"; a_disk="$(rand_jitter 27.0 2.0)"

  jq -n \
    --arg ts "${ts}" --arg k3s "${K3S_VERSION}" --argjson seq "${seq}" \
    --argjson scpu "${s_cpu}" --argjson smem "${s_mem}" --argjson sdisk "${s_disk}" \
    --argjson acpu "${a_cpu}" --argjson amem "${a_mem}" --argjson adisk "${a_disk}" \
    '{
      schema_version: "0.1.0",
      message_id: ("factory-c:infra_state:cluster:" + $ts),
      factory_id: "factory-c",
      ...
      payload: {
        ...
        cluster: { cluster_name: "factory-c", kubernetes_version: $k3s },
        nodes: [
          {
            node_id: "factory-c-master",
            role: "control-plane",
            ready: true,
            cpu_usage_percent: $scpu,
            memory_usage_percent: $smem,
            disk_usage_percent: $sdisk,
            network_reachability: "ok"
          },
          {
            node_id: "factory-c-worker",
            role: "worker",
            ready: true,
            cpu_usage_percent: $acpu,
            memory_usage_percent: $amem,
            disk_usage_percent: $adisk,
            network_reachability: "ok"
          }
        ],
        workloads: [],
...
[Unit]
Description=Aegis factory-c dummy publisher (testbed, worker VM)
Wants=network-online.target
After=network-online.target tailscaled.service k3s-agent.service

[Service]
Type=simple
EnvironmentFile=/etc/aegis/dummy-publisher.env
...
`mosquitto_pub` 은 성공 시 출력이 없다. journal 에 반복적인 에러가 없고 `publish-sequence` 가 20초마다 +1 되면 정상.
...
3. (60초 후) Task Scheduler `Start factory-c-worker VM` 실행 → worker VM 부팅 → K3s agent join + Tailscale + `aegis-dummy-publisher.service` active
...
- [ ] `/etc/aegis/dummy-publisher.env` K3S_VERSION 기록
...
### `aegis-dummy-publisher.service` 가 자주 재시작 (worker VM)

증상: `journalctl -u aegis-dummy-publisher` 에 mosquitto_pub 오류가 반복.

원인 후보:

- 인증서 경로/권한 → `/etc/aegis/iot/factory-c/` 600 권한, `endpoint.txt` 내용 확인
- `EnvironmentFile=/etc/aegis/dummy-publisher.env` 누락 또는 K3S_VERSION 미설정 → `cat /etc/aegis/dummy-publisher.env`
...
systemctl status aegis-dummy-publisher --no-pager   # worker VM
journalctl -u k3s -n 100 --no-pager
journalctl -u k3s-agent -n 100 --no-pager
journalctl -u aegis-dummy-publisher -n 100 --no-pager
```
...
- publisher 실패는 위 § "aegis-dummy-publisher.service 가 자주 재시작" 항목 참고

### Flannel 멀티노드 Pod 간 통신 실패 및 CoreDNS 502/DNS Resolution 오류

**증상**:
- `edge-iot-publisher` Pod에서 AWS IoT Core 엔드포인트 도메인 해석에 실패하여 `Temporary failure in name resolution` 또는 `MQTT connection failed` 오류가 반복 발생.
- CoreDNS Pod가 Running 상태이나 Master-Worker 간 Pod-to-Pod 네트워크 통신(VXLAN)이 작동하지 않아 Worker Pod의 DNS 요청이 CoreDNS로 도달하지 못함.

**원인**:
- VirtualBox의 기본 NAT 네트워크 어댑터(`enp0s3`)를 사용하면 Master와 Worker VM 모두 `INTERNAL-IP`가 `10.0.2.15`로 동일하게 중복 할당되어 K3s Flannel VXLAN 터널링이 꼬이거나 무시됩니다.

**해결**:
1. 각 VM에 호스트 전용 어댑터(Host-only Adapter) 또는 브릿지 어댑터를 추가하여 독립적인 고유 IP 대역을 확보합니다 (예: `enp0s8` 인터페이스, Master `192.168.56.10`, Worker `192.168.56.20`).
2. K3s 설치 시 `INSTALL_K3S_EXEC` 인자나 `/etc/rancher/k3s/config.yaml` 파일에 해당 고유 IP와 flannel 인터페이스를 반드시 강제 지정해 주어야 합니다.
   - **Master VM (`/etc/rancher/k3s/config.yaml`)**:
     ```yaml
     node-ip: 192.168.56.10
     flannel-iface: enp0s8
     ```
   - **Worker VM (`/etc/rancher/k3s/config.yaml`)**:
     ```yaml
     node-ip: 192.168.56.20
     flannel-iface: enp0s8
     ```
3. 설정 파일 반영 또는 `INSTALL_K3S_EXEC` 수정 후 K3s 서비스를 재시작합니다:
   ```bash
   # Master
   ...
   ```
4. `kubectl get nodes -o wide`를 통해 각 노드의 `INTERNAL-IP`가 중복된 `10.0.2.15` 대신 `192.168.56.x` 대역으로 올바르게 분리 및 표기되는지 확인합니다.

### 엣지 서버와 클라우드간의 시각 비동기화 (Clock Drift) 현상

**증상**:
- 더미 생성기(`dummy-generator`)가 발행하는 JSON 파일명 및 메시지 페이로드의 타임스탬프가 현재 실제 시간(예: S3 LastModified 수정 시간)보다 느리게 찍힘 (예: 30분 이상 차이 발생).
- 파일명 타임스탬프 시각 기준으로 S3 디렉토리가 생성되므로, 데이터가 예전 날짜/시간의 폴더로 적재되어 실시간 모니터링 정합성이 깨짐.

**원인**:
- 가상머신(VM)의 특성상 호스트 하드웨어의 클럭 전파 주기 지연이나 동기화 미비로 인해 시간 오차(Clock Drift)가 지속적으로 누적됩니다.
- NTP 데몬(`chrony` 등)이 돌고 있더라도, 클럭 오차 수준이 너무 크면(`maxdistance` 임계값 초과 등) 자동으로 시각을 조율하지 않고 차단해버릴 수 있습니다.

**해결**:
1. 동기화 대상 노드(예: Factory B의 `worker1`)에 접속하여 `chrony` 상태를 점검합니다:
   ```bash
   chronyc tracking
   ```
2. 누적 오차(`System time`)가 크거나 `unreachable` 상태인 경우, 수동으로 NTP 클럭 메인스텝(시간 강제 도약 동기화)을 실행합니다:
   ```bash
   sudo chronyc makestep
   ```
3. 정상적으로 실제 시간과 일치하는지 `date` 명령을 통해 검증하고, 더미 생성기 로그(`journalctl -u aegis-factory-b-dummy-generator`)에서 정상 시간으로 파일이 써지는지 확인합니다.
