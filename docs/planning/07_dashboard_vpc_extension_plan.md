...
Dashboard VPC가 Spoke나 Processing VPC 내부 API를 직접 조회하지 않으므로, 현장 상태도 Edge data-plane이 데이터 플레인으로 올려야 한다.
...
```json
{
  "message_id": "factory-a:infra_state:cluster:2026-05-15T01:00:00Z",
  "factory_id": "factory-a",
  "node_id": "cluster",
  "source_type": "infra_state",
  "source_timestamp": "2026-05-15T01:00:00Z",
  "published_at": "2026-05-15T01:00:02Z",
  "payload": {
    "heartbeat": {
      "agent_status": "alive",
      "last_successful_publish_at": "2026-05-15T01:00:02Z"
    },
    "nodes": [
      {
        "node_id": "worker2",
...
상태 데이터가 추가되면 메시지 수는 증가하지만, `infra_state`는 작은 JSON이다. MVP 규모에서는 주기와 payload 크기를 제한하면 IoT Core 병목은 관리 가능하다.
...
factory_state:
  주기: 3초
  payload: 센서/AI/audio 요약

infra_state:
  주기: 20초
  payload: heartbeat, 노드, 워크로드, 장치 요약
```
...
| --- | ---: | ---: |
| `infra_state` heartbeat | 20초 | 20~40초 |
| `infra_state` node/workload 상태 | 20초 | 20~40초 |
