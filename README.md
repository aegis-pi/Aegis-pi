...
| 현재 완료 범위 | M0 `factory-a`, M1 Issue 0~4, M1 Issue 5 IoT Thing/certificate/policy 및 K3s Secret 등록 |
| 현재 AWS 상태 | 2026-05-04 전체 destroy 완료. Hub EKS/VPC/namespace/ArgoCD deleted, foundation S3 bucket deleted, `factory-a` IoT/K3s Secret deleted |
| 후속 범위 | IoT Rule -> S3 적재, AMP, Dashboard VPC, Edge Agent, `factory-b`, `factory-c`, Risk Twin |
...
M1 Issue 0~4에서는 AWS MFA/Terraform 접근, Hub EKS/VPC 기준선, Hub namespace 기준선, Hub ArgoCD bootstrap, foundation S3 data bucket `aegis-bucket-data`를 검증했다. 이후 2026-05-04에 `DESTROY_FOUNDATION=true scripts/destroy/destroy-all.sh`로 Hub, foundation, `factory-a` IoT/K3s Secret을 전체 삭제했다. M1 Issue 5에서는 `factory-a` IoT Thing/certificate/policy와 K3s Secret 등록을 검증했고, 다음 작업은 필요 시 재생성 후 IoT Rule -> S3 `raw/` prefix 적재 연결이다.
...
| 경로 | 역할 | 현재 상태 |
| --- | --- | --- |
| `infra/hub` | VPC, subnet, NAT Gateway, EKS cluster, node group | destroy 완료, Terraform state empty |
| `scripts/ansible/playbooks/hub_argocd_bootstrap.yml` | kubeconfig 갱신, namespace, LimitRange, ArgoCD Helm install, 검증 | bootstrap/verify 완료 |
| `infra/foundation` | S3, ECR, AMP, IoT Core처럼 EKS destroy와 분리할 영속 리소스 | destroy 완료, Terraform state empty |
...
| Phase 2 (M1) | AWS EKS Hub 기준선 구성 | 진행 중, Issue 0~4 완료, Issue 5 부분 완료 |
