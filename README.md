...
| 현재 단계 | M2 Mesh VPN + Hub-Spoke 연결 |
| 현재 완료 범위 | M0 `factory-a`, M1 Issue 0~10/12, M2 Issue 1~2 완료. IoT Rule -> S3 raw 적재, IRSA S3/AMP 권한, Hub Prometheus Agent -> AMP remote_write, 내부 Grafana -> AMP query, AWS Load Balancer Controller, Admin UI HTTPS Ingress, `factory-a-master` Tailnet 참여와 Windows 운영자 PC SSH 검증 완료 |
...
| 다음 작업 | M2 Issue 3 EKS Hub Tailscale 참여 및 `factory-a-master` Tailscale IP reachability 검증. M1 Issue 11 운영 보안 강화는 MVP 이후로 보류 |
...
2026-05-07 기준 Tailscale Tailnet policy/tag 기준을 적용했고, 운영형 Spoke 대표 노드인 `factory-a-master`를 Tailnet에 참여시켰다. Worker 노드는 초기 M2 참여 대상에서 제외한다.
...
`factory-a-master`에는 `tag:aegis-spoke-prod`, `tag:factory-a`를 적용했다. Windows 운영자 PC에서 `100.117.40.125` ping 및 SSH 접근을 확인했다. 다음 단계는 EKS Hub를 Tailscale Kubernetes Operator 방식으로 Tailnet에 참여시키고 Hub 내부에서 `factory-a-master` Tailscale IP 접근을 검증하는 것이다.
