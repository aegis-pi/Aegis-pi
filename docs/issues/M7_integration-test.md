...
- Dashboard가 Spoke API나 Processing VPC 내부 API를 직접 조회하지 않음
...
- Dashboard VPC에서 latest status store 기반으로 Failover 상태 확인 가능
- 복구 후 관제 화면 자동 정상화
...
- [ ] `dashboard-vpc.md` - Dashboard VPC, ALB/WAF/Auth, read-only 조회 경계 반영
