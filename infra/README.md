...
| `data-pipeline/` | IoT Rule × 3, DataProcessor/Refresh, GraphAggregator5m, CloudInfra collectors, RiskAlertDispatcher, Scheduler/IAM/EKS access. on-demand 레이어. `build-data-pipe.sh` / `destroy-data-pipe.sh`로 개별 관리 |
...
- Hub 재생성은 `infra/hub` Terraform apply 후 유지 중인 SlowCollector EKS access binding을 자동 복구하고, `scripts/ansible/playbooks/hub_argocd_bootstrap.yml`, `hub_prometheus_agent_cleanup.yml`, `hub_grafana_bootstrap.yml`, `hub_aws_load_balancer_controller_bootstrap.yml` 순서로 진행한다. Admin UI Ingress와 Spoke 등록은 별도 후속 스크립트로 실행한다.
- Hub 삭제는 `scripts/destroy/destroy-hub.sh` 기준으로 Admin UI Ingress/ALB cleanup을 먼저 시도한 뒤 `infra/hub` Terraform destroy로 클러스터와 VPC를 제거한다.
- 전체 삭제는 `scripts/destroy/destroy-all.sh`를 사용한다. 기본 동작은 reporting → data-pipeline → Hub 순서이며, IoT/Foundation은 명시적 플래그(`DESTROY_IOT=true`, `DESTROY_FOUNDATION=true`)를 지정해야 삭제한다.
...
- Hub-only 데이터 수집 유지 모드에서는 data-pipeline을 destroy하지 않는다. 새 Hub 생성 후 `build-hub.sh`가 SlowCollector EKS access entry와 view policy association만 복구한다.
